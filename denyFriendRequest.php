...
checkpoint(isset($_POST["target"]), "Provide a target to deny as a friend!");

// Insert message
$query = $db->prepare("UPDATE `Friends` SET `RequestStatus`='DENIED' WHERE User2 = (SELECT UserID FROM User WHERE Name = ? AND Password = ?) AND User1 = (SELECT UserID FROM User WHERE Name = ?) AND `RequestStatus` = 'ACTIVE'");
